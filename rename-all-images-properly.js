...
for (const match of projectBlocks) {
  ...
  for (const imgInfo of imagePaths) {
    ...
    // Check if file exists or was already renamed
    let actualFile = imgFile;
    if (!fs.existsSync(imgPathFull)) {
      // Check if it was already renamed
      const files = fs.readdirSync(projectFolder);
      const alreadyRenamed = files.find(f => f.includes(slug) && !f.includes('thumbnail'));
      if (alreadyRenamed) {
  ...
  for (const mapping of mappings) {
    ...
    if (fs.existsSync(oldPath)) {
      if (!fs.existsSync(newPath)) {
        ...
        console.log(`   ⚠️  ${mapping.new} already exists, skipping ${mapping.old}`);
      }
