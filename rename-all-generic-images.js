...
console.log('🔄 Renaming ALL generic image files...\n');
...
for (const match of projectBlocks) {
  ...
  // Get all image references in order (thumbnail first, then gallery)
  const allImageRefs = [];
  ...
  for (const imgInfo of allImageRefs) {
    ...
    if (fs.existsSync(imgPathFull) && imgFile !== newName) {
      ...
      // Ensure uniqueness if new name already exists
...
console.log('✅ All generic images renamed and project data updated!');
