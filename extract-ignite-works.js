...
const url = 'https://cheetah-accordion-l9n5.squarespace.com/#/new-page/';
...
async function extractIgniteWorks() {
  console.log(`\n🔍 Extracting Ignite Works from: ${url}\n`);
  ...
  try {
    ...
    await page.waitForTimeout(8000); // Wait for hash navigation
    
    // Wait for project content
    await page.waitForSelector('h2, .project-description, .sqs-block-content', { timeout: 15000 }).catch(() => {});
    ...
    // Download images
    const projectImageDir = path.join(__dirname, 'public', 'images', 'projects', 'ignite-works');
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      try {
        ...
        downloadedImages.push({
          ...img,
          localPath: `ignite-works/${filename}`,
          downloaded: true,
        });
    ...
    const outputFile = path.join(__dirname, 'portfolio-extraction', 'ignite-works.json');
...
extractIgniteWorks().catch(console.error);
